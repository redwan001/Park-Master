 ...
 public class LineFollower : MonoBehaviour {
    ...
    public bool endPointReached;
     ...
     void FollowClumsy(){
       ...
       // Quaternion lTargetDir = Quaternion.Euler(Current(currentPoint) - transform.position);
        lTargetDir.y = 0.0f;
        transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(lTargetDir), Time.time * 0.04f);

       // transform.rotation = Quaternion.Slerp(transform.rotation, lTargetDir, Time.time * .2f);
